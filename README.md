...
|---|---|---|
| `--cream` | `#FCF8F1` | 배경 |
| `--hero` | `#F3EBDF` | 히어로 배경 — 키비주얼이 녹아드는 바탕색 |
| `--primary` | `#B9502F` | 테라코타 — CTA, 강조 |
| `--primary-deep` | `#9F432A` | 호버 · 작은 링크 텍스트 |
| `--sage` | `#465747` | 세이지 — 근거·안전 |
| `--sky` | `#8C725B` | 신부전 |
| `--honey` | `#C68A2F` | 췌장염 · 별점 |
| `--mauve` | `#9A6657` | 피부병 |
| `--ink-soft` / `--ink-faint` | `#5E6157` / `#6E7065` | 본문 보조 · 메타 |

색은 크림 배경 기준 WCAG AA(4.5:1)를 넘도록 잡았습니다 —
`--primary`는 흰 글씨 버튼에서 4.9:1, `--ink-faint`는 4.7:1입니다.
...
`HeroArt.tsx`는 사진을 액자에 넣지 않고 섹션 배경으로 흘려보냅니다.
데스크톱은 오른쪽 화면 끝까지 풀블리드로 깔고 왼쪽을 `--hero` 색
그라데이션으로 덮어 경계를 지우고, 모바일은 카피 아래 전폭 밴드로
이어 붙입니다. 원본 사진의 왼쪽 여백이 같은 크림 톤이라 이음매가
보이지 않습니다.

### 로고

`public/brand/logo.svg`(가로형) · `mark.svg`(심볼). 워드마크는
Pretendard ExtraBold, 심볼은 고양이 머리 + 하트입니다. 이름에
'고양이'가 없으므로 `무병장수` 아래 `고양이 처방식 · 건강식`
디스크립터를 늘 함께 씁니다.
